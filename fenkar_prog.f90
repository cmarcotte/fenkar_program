	...
	DO iz = 1,nz
		DO iy = 1,ny
			DO ix = 1,nx
				...
				th = ATAN2(yy,xx)
				x(ix,iy,iz,1) = 0.00 + 1.0*H(xx-0.7,10.0)
				x(ix,iy,iz,2) = 1.00 - 1.0*H(yy-0.3,10.0)
				x(ix,iy,iz,3) = 1.00 - 1.0*H(zz-0.5,2.0)
...
IMPLICIT NONE
	...
	wt = Int(ceiling(TT/dt))	! wt*dt == time interval
	...
	dxdt(:,:,:,:) = 0.0
	p = (/ 30.0, 50.0, 19.6, 3.33, 41.0, 471.0, 0.25, 12.5, 33.0, 10.0, 0.13, 0.04, 0.85 /)
	t = t0
